...
import { StatutCredit, StatutRetrait, StatutTransaction, TypeTransaction } from '@prisma/client';
...
export class RapportsService {
  ...
  async exportTransactionsCsv(dto: FiltrerRapportDto) {
    ...
    const csv = this.csv(
      ['reference', 'date', 'client', 'telephone', 'role', 'tontine', 'type', 'statut', 'montant', 'montantNet', 'fraisPlateforme', 'fraisAgent', 'operateur', 'refKKiaPay'],
      transactions.map((tx) => [
        tx.reference,
  ...
  async exportRetraitsCsv(dto: FiltrerRapportDto) {
    ...
    const csv = this.csv(
      ['id', 'date', 'client', 'telephone', 'role', 'tontine', 'montant', 'statut', 'validePar', 'motifRejet', 'refKKiaPay', 'executeLe'],
      retraits.map((r) => [
        r.id,
  ...
  async exportMicroCreditsCsv(dto: FiltrerRapportDto) {
    ...
    const csv = this.csv(
      ['id', 'date', 'client', 'telephone', 'collecteurId', 'principal', 'interet', 'total', 'journalier', 'joursPayes', 'restant', 'statut', 'score', 'echeance'],
      credits.map((c) => [
        c.id,
  ...
  async rapportFinancierPdf(dto: FiltrerRapportDto) {
    ...
    const [
      ...
      this.prisma.transaction.aggregate({
        where: { type: TypeTransaction.COTISATION, statut: StatutTransaction.SUCCES, creeLe: { gte: periode.debut, lte: periode.fin } },
        _sum: { montant: true, montantNet: true, fraisPlateforme: true, fraisAgent: true },
        _count: true,
      }),
      this.prisma.retrait.aggregate({
        where: { statut: StatutRetrait.EXECUTE, creeLe: { gte: periode.debut, lte: periode.fin } },
        _sum: { montant: true },
        _count: true,
      }),
      this.prisma.transaction.aggregate({
        where: { type: TypeTransaction.DISTRIBUTION_GROUPE, creeLe: { gte: periode.debut, lte: periode.fin } },
      ...
      this.prisma.microCredit.findMany({
        where: { creeLe: { gte: periode.debut, lte: periode.fin }, statut: { in: [StatutCredit.ACTIF, StatutCredit.TERMINE, StatutCredit.EN_DEFAUT] } },
      ...
      this.prisma.transaction.aggregate({
        where: { type: TypeTransaction.ABONNEMENT, statut: StatutTransaction.SUCCES, creeLe: { gte: periode.debut, lte: periode.fin } },
        _sum: { montant: true },
        _count: true,
      }),
    ...
    const creditsEnDefaut = microCredits.filter((c) => c.statut === StatutCredit.EN_DEFAUT).length;
    ...
    const agents = await this.prisma.utilisateur.findMany({
      ...
      include: {
        clients: {
          include: {
            transactions: {
              where: { type: 'COTISATION' as any, statut: 'SUCCES' as any, creeLe: { gte: periode.debut, lte: periode.fin } },
              select: { montant: true },
            },
  ...
  async bilanComptablePdf(dto: FiltrerRapportDto): Promise<{ buffer: Buffer; filename: string }> {
    const periode = this.periode(dto);
    ...
    const [cotisations, retraits, credits, commissions, decaissements] = await Promise.all([
      this.prisma.transaction.aggregate({ where: { ...where, type: 'COTISATION' as any, statut: 'SUCCES' as any }, _sum: { montant: true, fraisPlateforme: true }, _count: true }),
      this.prisma.transaction.aggregate({ where: { ...where, type: 'RETRAIT' as any, statut: 'SUCCES' as any }, _sum: { montant: true }, _count: true }),
      this.prisma.microCredit.findMany({ where: { ...where, statut: { in: ['ACTIF', 'TERMINE'] as any } }, select: { montantPrincipal: true, montantTotal: true } }),
      this.prisma.commission.aggregate({ where, _sum: { montant: true }, _count: true }),
      this.prisma.transaction.aggregate({ where: { ...where, type: 'DEBLOCAGE_CREDIT' as any, statut: 'SUCCES' as any }, _sum: { montant: true } }),
    ]);
