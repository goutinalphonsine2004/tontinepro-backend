...
let PadmeService = class PadmeService {
    ...
    async pdf(dossierId, utilisateurId, role) {
        const dossier = await this.prisma.dossierPADME.findUnique({
            where: { id: dossierId },
        ...
        if (!dossier)
            throw new common_1.NotFoundException('Dossier PADME introuvable');
        const estAdmin = role === client_1.Role.ADMIN || role === client_1.Role.SUPERVISEUR;
        if (!estAdmin && dossier.clientId !== utilisateurId) {
            throw new common_1.ForbiddenException('Accès refusé à ce dossier PADME');
        }
        if (!dossier.urlPDF) {
            throw new common_1.NotFoundException('PDF PADME non généré pour ce dossier');
        }
        const cheminNormalise = (0, path_1.normalize)(dossier.urlPDF);
        if (cheminNormalise.startsWith('..') || cheminNormalise.startsWith('/')) {
            throw new common_1.BadRequestException('Chemin PDF invalide');
        }
        const chemin = (0, path_1.join)(process.cwd(), cheminNormalise);
        try {
            const buffer = await (0, promises_1.readFile)(chemin);
            return { buffer, filename: (0, path_1.basename)(chemin) };
        }
        catch {
            throw new common_1.NotFoundException('Fichier PDF PADME introuvable');
        }
    }
