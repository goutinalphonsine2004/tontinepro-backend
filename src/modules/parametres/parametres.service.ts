...
export class ParametresService {
  ...
  async get(cle: string) {
    if (!DEFAULTS[cle]) {
      throw new NotFoundException({ message: `Paramètre "${cle}" inconnu`, code: 'PARAMETRE_INCONNU' });
    }
  ...
  async set(cle: string, dto: SetParametreDto, adminId: string) {
    ...
    if (cle === 'MODE_MAINTENANCE' && !['true', 'false'].includes(dto.valeur)) {
      throw new BadRequestException('La valeur du mode maintenance doit être "true" ou "false"');
    }
