...
export class RetraitsService {
  ...
  async demanderOtp(utilisateurId: string, dto: DemanderRetraitDto) {
    ...
    const { code, expireLe } = await this.creerOtpRetrait(
      utilisateurId,
      telephone,
      dto.tontineId,
      dto.montant,
  ...
  private async verifierAucuneAlerteBloquante(tontineId: string) {
    ...
    if (alerte) {
      throw new ForbiddenException({
        message:
          'Retrait temporairement bloqué: anomalie comptable en cours de vérification.',
  ...
  private async creerOtpRetrait(
    ...
    const expireLe = new Date(
      Date.now() + DUREE_OTP_RETRAIT_MINUTES * 60 * 1000,
    );
